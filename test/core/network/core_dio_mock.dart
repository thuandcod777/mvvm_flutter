...
class CoreDioMock with DioMixin implements Dio, ICoreDioFullNulSafetyNull {
  ...
  },
  {
    ...
    final model = _responseParse<R, T>(parseModel, response);
  ...
  R? _responseParse<R, T>(BaseModel? model, dynamic data) {
    if (data is List) {
